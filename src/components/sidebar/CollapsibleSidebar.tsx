...
export function CollapsibleSidebar({
  ...
  return (
    <div className={`collapsible-sidebar relative h-full ${className}`.trim()}>
      ...
      {/* Sidebar panel: fixed width, slides right when closed; swipe from handle to close */}
      <motion.div
        className="collapsible-sidebar__panel absolute right-0 top-0 z-20 flex h-full min-w-96 max-w-[min(28rem,50%)] w-1/2"
        initial={false}
        ...
        dragListener={false}
      >
        {/* Swipe handle: drag right to close (iPad / touch) */}
        <div
